...
- LLM-as-judge rubric scoring via structured output (§6 B-2, D-5): severity match, root-cause match, plan actionability, gate compliance.
...
## Acceptance
- `benchmark` command runs locally (Redis + OpenRouter models, D-5); emits the report. `--smoke` = deterministic fakes, zero tokens, CI-safe.
